...
export default async function uploadConfirmedImage(
  uri: string,
  ...
  if (error) {
    throw new Error(
      `Error uploading photo to Supabase Storage: ${error.message}`,
    );
  }
  ...
  if (insertError) {
    throw new Error(
      `Error inserting image reference into database: ${insertError.message}`,
    );
  }
